...
const parseRideDateTime = (dateStr: string, timeStr: string): Date | null => {
  try {
    // Parse date like "January 16" - we need to handle potential different years
    const now = new Date();
    ...
    // If parsing fails or date seems wrong (like future date in wrong year), try last year
    if (isNaN(parsedDate.getTime()) || parsedDate.getTime() > now.getTime()) {
      const lastYearDateTimeStr = `${dateStr}, ${currentYear - 1} ${timeStr}`;
      parsedDate = new Date(lastYearDateTimeStr);
    }

    // If parsing still fails, try alternative formats
      ...
      if (timeParts) {
        ...
        if (dateParts) {
          ...
          if (monthIndex !== -1) {
            ...
            // If date is in the future (like December when it's currently January), try last year
...
const shouldHideRideBasedOnDateTime = (ride: Ride): boolean => {
  try {
    ...
    // Hide if ride is from last year (any time last year)
    if (rideYear < currentYear) {
      console.log(
        `Hiding ride ${ride.id} because it's from year ${rideYear} (current year: ${currentYear})`,
      );
      return true;
    }
    ...
    if (daysSinceRide > 5) {
      console.log(
        `Hiding ride ${ride.id} because it's ${daysSinceRide} days old`,
      );
      return true;
    }
...
const shouldArchiveRideBasedOnDateTime = async (
  ...
  try {
    ...
    // Archive if ride is from last year (any time last year)
    if (rideYear < currentYear) {
...
export default function HomeScreen() {
  ...
  const checkAndArchiveOldRides = async (rideData: Ride[]) => {
    try {
      console.log(`Checking ${rideData.length} rides for archiving...`);
      let archivedCount = 0;
      ...
      for (const ride of rideData) {
        ...
        if (shouldArchive) {
          console.log(`Archiving ride: ${ride.id} - ${ride.date} ${ride.time}`);
      ...
      if (archivedCount > 0) {
        console.log(`✅ Archived ${archivedCount} rides`);
      }
    } catch (error) {
  ...
  const setupRealTimeListener = () => {
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        try {
          console.log(
            `Real-time update received: ${snapshot.docs.length} rides`,
          );
          ...
          // Log rides from last year for debugging
          rideData.forEach((ride) => {
            if (!ride.archived) {
              const rideDateTime = parseRideDateTime(ride.date, ride.time);
              if (rideDateTime) {
                ...
                if (rideYear < currentYear) {
                  console.log(
                    `Found unarchived ride from last year: ${ride.id} - ${ride.date} ${ride.time} (year: ${rideYear})`,
                  );
                }
              }
            }
  ...
  const fetchRidesManually = async () => {
    try {
      console.log("Fetching rides manually...");
      const rideQuery = query(
        collection(db, "rides"),
        orderBy("createdAt", sortOrder === "newest" ? "desc" : "asc"),
      );
  ...
  const filteredRides = rides.filter((ride) => {
    ...
    // Hide rides that are more than 5 days old OR from last year
